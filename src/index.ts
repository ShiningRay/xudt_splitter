...
async function splitCell() {
  try {
    ...
    if (!targetXudtCells || targetXudtCells.length === 0) {
      ...
      // 获取每一个xudt cell的xudt数量
      for (let i = 0; i < targetXudtCells.length; i++) {
        const cell = targetXudtCells[i]
        const xudtAmount = leToU128(cell.outputData)
        console.log(`xudtAmount: ${xudtAmount}`)
        // 如果xudt数量小于QUANTITY_PER_CELL,就不纳入targetXudtCellsCount的统计
        if (xudtAmount < BigInt(process.env.QUANTITY_PER_CELL)) {
          continue
        }
        targetXudtCellsCount++
      }
    }
    ...
    const fromXudtBalance = fromXudtCells.reduce((prev, current) => {
      // 检查current.outputData是否只包含"0x"
      if (current.outputData === "0x" || current.outputData === "") {
        return prev; // 如果是，就直接返回之前的累加结果
      }
      return prev + BigInt(leToU128(current.outputData))
    }, BigInt(0));
